...
function accessSecret() {
  return byId("accessSecret").value;
}

function requireAccessSecret() {
  const secret = accessSecret();
  if (state.health?.demo_access_required && !secret) throw new Error("Enter the deployed demo access secret.");
  return secret;
}
...
async function startRun(event) {
  ...
  try { const run = await api.startRun({ order_id: byId("orderSelect").value, mode: byId("modeSelect").value, scenario: byId("scenarioSelect").value }, requireAccessSecret()); await loadTrace(run.trace_id); }
  catch (error) {
...
async function resolveApproval(decision, controls) {
  ...
  try {
    ...
    await api.decideApproval(state.approval.approval_id, { decision, approver_label: controls.approverInput.value, comment: controls.commentInput.value || null, expected_version: state.approval.version, idempotency_key: idempotencyKey(`dashboard-${decision}`) }, secret, requireAccessSecret());
...
async function createDisposable() {
  ...
  try { const run = await api.startRun({ order_id: "ORD-8842", mode: "enforce", scenario: "benign" }, requireAccessSecret()); const verification = await api.verify(run.trace_id); update({ disposableTraceId: run.trace_id, disposableVerification: verification }); renderDisposable(run.trace_id, verification); byId("tamperButton").disabled = false; renderMessage("Disposable trace is clean and ready for the integrity demo."); }
...
async function initialize() {
  ...
  try {
    ...
    const accessInput = byId("accessSecret"); accessInput.value = sessionStorage.getItem("manifestDemoAccessSecret") || ""; byId("accessSecretLabel").classList.toggle("hidden", !health.demo_access_required); accessInput.addEventListener("input", () => sessionStorage.setItem("manifestDemoAccessSecret", accessInput.value));
